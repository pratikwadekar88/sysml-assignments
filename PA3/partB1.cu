...
int main() {
    ...
    float *h_b1 = (float*)malloc(H1 * sizeof(float));
    float *h_b2 = (float*)malloc(H2 * sizeof(float));
    float *h_b3 = (float*)malloc(H3 * sizeof(float));
    float *h_b4 = (float*)malloc(OUT * sizeof(float));
    float *h_X  = (float*)malloc(N * IN * sizeof(float));
    float *h_y  = (float*)malloc(N * OUT * sizeof(float));
    ...
    // 4. Copy Initialized Data from Host to Device
    ...
    free(h_b1); free(h_b2); free(h_b3); free(h_b4);
    free(h_X); free(h_y);
    ...
    std::cout << "Avg Forward Pass Time: " << total_time_fw / 60000 << " ms\n";
